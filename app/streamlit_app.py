...
# Ensure project root is importable when Streamlit executes from `app/`.
...
def main():
    st.title("EduPro Forecasting")
    st.caption("Production inference app for enrollment, course revenue, and category revenue forecasts.")
    ...
    if metadata_error:
        st.warning(
            "Model metadata file was not found or unreadable. "
            "Using fallback metadata inferred from model files."
        )
    ...
    with st.sidebar:
        ...
        st.write(f"Data path: `{DEFAULT_DATA_PATH}`")
        st.write("Models loaded: enrollment, course revenue, category revenue")
        st.write(f"Known categories: {len(ref['categories'])}")
        st.write(f"Known courses: {len(ref['course_ids'])}")
    ...
    st.subheader("1) Course Profile")
    source_mode = st.radio("Course source", ["Existing course", "New course"], horizontal=True)
    ...
    with col_d:
        instr_experience_bucket = st.number_input(
            "Instructor Experience Bucket (0-5)",
            min_value=0.0,
            max_value=5.0,
    ...
    with col_f:
        instr_expertise_match = st.number_input(
            "Instructor Expertise Match (0-1)",
            min_value=0.0,
            max_value=1.0,
    ...
    st.subheader("2) Historical Signals")
    ...
    target_date = st.date_input("Forecast Month (any date within month)", value=default_target_month)
    ...
    if forecast_payload:
        ...
        with tab1:
            ...
            m1.metric("Next-Month Enrollments", f"{predictions['pred_enrollment']:.2f}")
            m2.metric("Next-Month Course Revenue", f"${predictions['pred_course_revenue']:.2f}")
            m3.metric("Next-Month Category Revenue", f"${predictions['pred_category_revenue']:.2f}")
            st.caption(
                "Demand forecast uses blended enrollment prediction from direct model and lag baseline."
            )
            ...
            revenue_chart_df = pd.DataFrame(
                {
                    "Metric": [
                        ...
                        "Structural Revenue (Enrollment x Price)",
                        "Final Blended Revenue",
                    ],
                    "Value": [
                        predictions["pred_course_revenue_direct"],
                        predictions["pred_course_revenue_structural"],
                        predictions["pred_course_revenue"],
                    ],
                }
            )
            st.bar_chart(revenue_chart_df.set_index("Metric"))
            st.caption(
                f"Revenue blend alpha: {predictions['course_revenue_blend_alpha']:.2f}"
            )
        ...
        with tab3:
            ...
            if importance_df.empty:
                ...
                st.bar_chart(importance_df.head(top_n).set_index("feature"))
            ...
            category_comp_df = pd.DataFrame(
                {
                    ...
                    "Value": [
                        predictions["pred_enrollment"],
                        _safe_float(result_category_enrollment, 0.0),
                        median_course_demand,
                    ],
                }
            )
            st.bar_chart(category_comp_df.set_index("Metric"))
            st.metric("Predicted Course Share of Category", f"{predicted_share_pct:.2f}%")
            st.caption(
                "Category comparison uses current category total input and historical category median from reference data."
            )
        ...
        st.info("Set inputs and click `Generate Forecast` to view dashboard results.")
